...
# 왜 Equal Linear : 동일한 Linear layer를 생성해 주는 듯
# 직접 parameter와 bias를 만들어 준다.
...
class Generator(nn.Module):
    def __init__(
        ...
        layers = [PixelNorm()]
        # n개 만큼의 multi layer perceptron 형성
        # 하나의 multi layer perceptron안에는 linear layer + activation이 있다.
        # mapping network
        ...
        self.style = nn.Sequential(*layers) # style
    ...
    def mean_latent(self, n_latent):
        ...
        ) # torch.Size([n_latent, style_dim])
        
        
        latent = self.style(latent_in).mean(0, keepdim=True) # torch.Size([1, style_dim])

        return latent

    def get_latent(self, input):
        return self.style(input) # torch.Size([1, style_dim])
    ...
    def forward(
        ...
        if len(styles) < 2:
            ...
            # freezeStyle
            # latent의 일부를 random 하게 지정한다. 반대 : 일부를 고정한다.
            if put_latent is not None:
                latent1 = put_latent[:,:inject_index] # 차원 변환
