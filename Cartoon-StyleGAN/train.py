...
def requires_grad(model, flag=True, target_layer=None):
    for name, param  in model.named_parameters(): # 이걸로 정보를 얻어와서 freeze 가능하다.
        if target_layer is None or target_layer in name:
            param.requires_grad = flag

# 뭔가 model2의 데이터를 참조하여 model1의 parameter를 바꿔주는 함수?
# model2의 파라메터 값을 decay만큼 model1에 반영시키는 함수
def accumulate(model1, model2, decay=0.999):
    par1 = dict(model1.named_parameters())
    par2 = dict(model2.named_parameters())

    # par2[k].data들을 alpha만큼 비율로 par1에 더한다음에 decay만큼 곱해준다.
    # 그런데 왜?
    for k in par1.keys():
...
def train(args, loader, generator, generator_source, discriminator, g_optim, d_optim, g_ema, device):
    ...
    for idx in pbar:
        ...
        # next() : https://hgko1207.github.io/2020/08/13/ai-pytorch-2/ 
        real_img = next(loader)
        real_img = real_img.to(device)

        # freeze generator, discriminator 
        # 모든 레이어를 얼린다.
        ...
        if args.freezeStyle >= 0:            
            _, latent = generator_source(noise, return_latents=True) # generator_source에서 noise to image, latent vector 생성
        ...
        noise = mixing_noise(args.batch, args.latent, args.mixing, device)
        # noise -> fake img 만들기
        # freezeStyle 만큼의 style vector를 만들어 낸다. -> 여러 이미지가 나오나?
        # source image의 latent vector의 일부를 학습시키려는 generator에 넣어준다.
        if args.freezeStyle >= 0:
            # generator, generator_source에 같은 noise를 넣어준다.           
            fake_img, latent = generator_source(noise, return_latents=True)
            # generator_source에서 나온 latent vecotr를 generator에 넣어준다.
            fake_img, _ = generator(noise, inject_index=args.freezeStyle, put_latent = latent)
        else:
            fake_img, _ = generator(noise)
        ...
        # swap layer의 latent vector와 image 를 반환하는 듯
        # 1~n번째 layer의 source generator와 generator의 latent vector 거리 계산
        # 비슷한 structure을 가질 수 있도록 loss 지정
...
if __name__ == "__main__":
    ...
    g_ema.eval()
    # generator parameter들을 g_ema parameter에 뭔가를 해줌 
    # g_ema의 weight, bias를 초기화 한다?
    # Q. decay가 0이면 그냥 모든 parameter가 0이 된다?
    accumulate(g_ema, generator, 0)

    # Q. what?
    g_reg_ratio = args.g_reg_every / (args.g_reg_every + 1) # re
    d_reg_ratio = args.d_reg_every / (args.d_reg_every + 1)

    # generator와 discriminator의 optimizer를 정의
    g_optim = optim.Adam(
