...
def main():
  ...
  if os.path.isfile(args.input_latent_codes_path):
    ...
    latent_codes = np.load(args.input_latent_codes_path) # file load
    latent_codes = model.preprocess(latent_codes, **kwargs) # normalization 
  else:
  ...
  for sample_id in tqdm(range(total_num), leave=False):
    # linear_interpolate : output : letnet code
    # 핵심 부분
    interpolations = linear_interpolate(latent_codes[sample_id:sample_id + 1],
                                        ...
                                        steps=args.steps)
    interpolation_id = 0
    # generate image with editted vector & save
