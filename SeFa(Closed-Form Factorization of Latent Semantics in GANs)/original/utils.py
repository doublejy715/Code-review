...
def factorize_weight(generator, layer_idx='all'):
    ...
    Args:
        generator: Generator to factorize. factorize할 generator를 선택
        layer_idx: Indices of layers to interpret, especially for StyleGAN and
            StyleGAN2. (default: `all`) 해석한 layer를 선택
    ...
    if gan_type == 'pggan':
        ...
        else: # index 분석 : layer_idx에 해당하는 index만 분석한다. 분석 대상 layer가 전체 레이어 범위 안에 있는지 체크
    ...
    for idx in layers:
        ...
        if gan_type == 'pggan':
            ...
            weight = generator.synthesis.__getattr__(layer_name).style.weight.T # 원하는 layer의 style weight를 구하는 방법
