...
def load_generator(model_name):
    ...
    with dnnlib.util.open_url('checkpoints/network-snapshot-000400.pkl') as f:
...
def factorize_weight(generator, layer_idx='all'):
    ...
    #                                min_val=0,
    #                                max_val=18)
    layers = parse_indices(layer_idx, min_val=0, max_val=16)

    # Factorize semantics from weight.
    weights = []
    for idx in layers:
        ids = int((idx-1)/2 + 1)
        layer_name = f'b{generator.synthesis.block_resolutions[ids]}'
        if idx % 2 == 0:
            weight = generator.synthesis.__getattr__(layer_name).conv1.affine.weight.T # generator.synthesis.__getattr__ 로 접근 가능 .conv1.affine.weight.T
        else:
            weight = generator.synthesis.__getattr__(layer_name).conv0.affine.weight.T # generator.synthesis.__getattr__ 로 접근 가능 .conv1.affine.weight.T
