...
saveFolder = 'result/images'

counter = 0
labels = []
for img_name in sorted(os.listdir('data/images')):
    ...
    for i in range(9):
        ...
        cv2.imwrite(os.path.join(saveFolder, \
            f'{counter}.png'), resultLab)
        
        counter += 1
        label[i] = 1; labels.append(label)
        #----------------------------------------------
np.save('result/labels.npy',np.float32(labels))
print(np.float32(labels).shape)
print('JOBs DONE!!')