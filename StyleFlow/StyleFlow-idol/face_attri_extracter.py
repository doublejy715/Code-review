...
def MS_Face(image):
    ...
    face_attributes = ['age', 'gender', 'headPose', 'smile', 'facialHair', 'glasses', 'emotion','hair','emotion']
    ...
    for face in detected_faces:
        ...
        attributes = [[face.face_attributes.smile]]
...
for name in os.listdir('image/'):
    # Detect a face with attributes, returns a list[DetectedFace]
    detected_faces = face_client.face.detect_with_stream(open('images'+name,'rb'), return_face_attributes=face_attributes)
    if not detected_faces:
        raise Exception(
            'No face detected from image {}'.format(os.path.basename(image)))
    if len(detected_faces) > 1:
